...
bool calculateSalesRevenueByDateRange(MYSQL* conn, const std::string& startDate, const std::string& endDate) {
    ...
    std::string query = "SELECT DATE_FORMAT(rental_date, '%Y-%m-%d') AS rental_date, TotalPrice FROM rental WHERE rental_date BETWEEN '" + startDate + "' AND '" + endDate + "'";

    // Add the sum of rental_price to the query
    query += " UNION ALL SELECT 'Total', SUM(TotalPrice) FROM rental WHERE rental_date BETWEEN '" + startDate + "' AND '" + endDate + "'";
...
bool findMostRentedCarByDateRange(MYSQL* conn, const std::string& startDate, const std::string& endDate) {
    ...
    std::string query = "SELECT CarID, COUNT(*) AS rental_count FROM rental WHERE rental_date BETWEEN '" + startDate + "' AND '" + endDate + "' GROUP BY CarID ORDER BY rental_count DESC LIMIT 1";
